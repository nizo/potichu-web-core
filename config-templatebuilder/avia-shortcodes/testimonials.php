...
array(
							...
							"subtype" => array(	__('Testimonial Grid', 'avia_framework' ) =>'grid',
												...
												__('Potichu Testimonial Slider (Large Custom)', 'avia_framework' ) =>'slider_custom_large',
							)
						),
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				if($style == "slider_custom_large")
				{
					//$style = "slider";
					//$custom_class .= " av-large-testimonial-slider";
					//$controls = false;
					$this->config['potichu_custom_testimonial'] = true;
				}
				...
				if($style != "grid" && $style != 'slider_custom_large') avia_sc_testimonial::$columns = 100000;
	...
	//meta
				...
				if ($this->config['potichu_custom_testimonial']) {									
					
					$markup = avia_markup_helper(array('context' => 'person','echo'=>false, 'custom_markup'=>$custom_markup));
					
					
					$output .= "<div class='potichu_custom_testimonial avia-testimonial {$class}' $markup>";
		...
		if($subtitle)	$output .= 					"<h2  class='avia-testimonial-subtitle {$this->subtitle_class}' {$this->title_styling}  {$markup_job}>{$subtitle}</h2>";
