	...
	class avia_social_media_icons
	{
		...
		function build_icon($icon)
		{
			...
			switch($icon['social_icon'])
			{
				...
				case 'twitter':
				case 'dribbble':
				case 'gplus': $publisher = 'rel="publisher"'; break; 
				case 'vimeo':
				case 'behance':
			...
			$html .= "<a {$blank} href='".$icon['social_icon_link']."' ".av_icon_string($icon['social_icon']). ' ' . $publisher . " title='".ucfirst($icon['social_icon'])."'><span class='avia_hidden_link_text'>".ucfirst($icon['social_icon'])."</span></a>";
